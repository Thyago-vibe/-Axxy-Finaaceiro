...
export const PredictiveAnalysis: React.FC = () => {
  ...
  const handleAddVariable = () => {
    if (!newVariableName || !newVariableAmount) return;

    const amount = parseFloat(newVariableAmount);
    ...
    const newScenario: PredictionScenario = {
      ...
      savings: amount,
      checked: true,
      iconName: 'Plus', // Default icon for custom
      color: 'text-white'
    };
          ...
          <div className="space-y-4">
            ...
            {!isAddingVariable ? (
              <button
                ...
                className="flex items-center gap-2 text-axxy-primary text-sm font-bold hover:text-green-400 transition-colors mt-2"
              >
                <Plus size={16} /> Adicionar Variável
              </button>
            ) : (
              <div className="bg-[#15221c] border border-[#1e332a] rounded-xl p-4 mt-2 animate-fade-in space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Vender bicicleta"
                    className="w-full bg-[#0b120f] border border-gray-700 text-white rounded-lg py-2 px-3 text-sm focus:ring-1 focus:ring-axxy-primary outline-none"
                    autoFocus
                  />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Economia Mensal (R$)</label>
                  <input
                    type="number"
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 py-1.5 text-xs text-gray-400 hover:text-white transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                    onClick={handleAddVariable}
                    className="flex-1 bg-axxy-primary text-axxy-bg font-bold py-1.5 rounded-lg text-xs hover:bg-axxy-primaryHover transition-colors"
                  >
                    Adicionar
                  </button>
                </div>
            ...
            <div className="bg-[#15221c] border border-[#1e332a] rounded-3xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-300 text-sm leading-relaxed">
                  ...
                  aumentar em <span className="text-green-400 font-bold">R$ {projectedExtra.toLocaleString('pt-BR')}</span>.
